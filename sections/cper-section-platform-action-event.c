/**
 * Describes functions for converting Platform Action Event sections from binary
...
//Converts the given Platform Action Event CPER section into JSON IR.
json_object *cper_section_platform_action_event_to_ir(const UINT8 *section,
						      UINT32 size,
						      char **desc_string)
{
	int outstr_len = 0;
	...
	*desc_string = NULL;
	if (size < sizeof(EFI_PLATFORM_ACTION_EVENT)) {
		cper_print_log(
			"Error: Platform Action Event section too small\n");
		return NULL;
	}
	...
	*desc_string = calloc(1, SECTION_DESC_STRING_SIZE);
	...
	EFI_PLATFORM_ACTION_EVENT *action_event =
		(EFI_PLATFORM_ACTION_EVENT *)section;
	json_object *section_ir = json_object_new_object();

	//UINT16     ValidationBits;
	add_bool(section_ir, "recordIdValid",
		 !!(action_event->ValidationBits &
		    EFI_PLATFORM_ACTION_RECORD_ID_VALID));
	add_bool(section_ir, "sectionIndexValid",
		 !!(action_event->ValidationBits &
		    EFI_PLATFORM_ACTION_CPAD_SECTION_INDEX_VALID));
	add_bool(section_ir, "actionIdValid",
		 !!(action_event->ValidationBits &
		    EFI_PLATFORM_ACTION_ACTION_ID_VALID));
	add_bool(section_ir, "actionReturnCodeValid",
		 !!(action_event->ValidationBits &
		    EFI_PLATFORM_ACTION_ACTION_RETURN_CODE_VALID));
	add_bool(section_ir, "additionalContextValid",
		 !!(action_event->ValidationBits &
		    EFI_PLATFORM_ACTION_ADDITIONAL_CONTEXT_VALID));

	//UINT8      ActionReturnCode;
	if (action_event->ValidationBits &
	    EFI_PLATFORM_ACTION_ACTION_RETURN_CODE_VALID) {
		add_int_hex_8(section_ir, "actionReturnCode",
			      action_event->ActionReturnCode);
		add_int_hex_8(section_ir, "actionReturnReasonCode",
			      action_event->ActionReturnReasonCode);
	}
	...
	if (action_event->ValidationBits &
	    EFI_PLATFORM_ACTION_RECORD_ID_VALID) {
		add_int_hex_64(section_ir, "cpadRecordId",
			       action_event->CpadRecordId);
	}

	//UINT16     CpadActionId;
	if (action_event->ValidationBits &
	    EFI_PLATFORM_ACTION_ACTION_ID_VALID) {
		add_int_hex_16(section_ir, "cpadActionId",
			       action_event->CpadActionId);
	}

	//UINT32     CpadSectionDescriptorIndex;
	if (action_event->ValidationBits &
	    EFI_PLATFORM_ACTION_CPAD_SECTION_INDEX_VALID) {
		add_uint(section_ir, "cpadSectionIndex",
			 action_event->CpadSectionDescriptorIndex);
	}

	size_t additional_context_size =
		size - sizeof(EFI_PLATFORM_ACTION_EVENT);
	...
	if (action_event->ValidationBits &
		...
		if (additional_context_size > 0) {
			INT32 encoded_len = 0;
			char *encoded_context = base64_encode(
				(const UINT8 *)(action_event) +
					sizeof(EFI_PLATFORM_ACTION_EVENT),
				additional_context_size, &encoded_len);

			if (encoded_context == NULL) {
				cper_print_log(
					"Error: Failed to allocate encode output buffer for Platform Action Event additional context. \n");
			} else {
				add_string_len(section_ir, "additionalContext",
					       encoded_context, encoded_len);
...
//Converts a single Platform Action Event JSON IR section into CPER binary, outputting to the given stream.
void ir_section_platform_action_event_to_cper(json_object *section, FILE *out)
{
	EFI_PLATFORM_ACTION_EVENT *section_cper =
		calloc(1, sizeof(EFI_PLATFORM_ACTION_EVENT));
	...
	int additional_context_valid =
		0; // remember if there is additional context after freeing section_cper
	...
	if (json_object_object_get_ex(section, "sectionIndexValid",
				      &valid_obj) &&
	    json_object_get_boolean(valid_obj)) {
		section_cper->ValidationBits |=
			EFI_PLATFORM_ACTION_CPAD_SECTION_INDEX_VALID;
	...
	if (json_object_object_get_ex(section, "actionReturnCodeValid",
				      &valid_obj) &&
	    json_object_get_boolean(valid_obj)) {
		section_cper->ValidationBits |=
			EFI_PLATFORM_ACTION_ACTION_RETURN_CODE_VALID;
	}
	if (json_object_object_get_ex(section, "additionalContextValid",
				      &valid_obj) &&
	    json_object_get_boolean(valid_obj)) {
		section_cper->ValidationBits |=
			EFI_PLATFORM_ACTION_ADDITIONAL_CONTEXT_VALID;
	...
	// Action Return Code
	get_value_hex_8(section, "actionReturnCode",
			&section_cper->ActionReturnCode);

	// Action Return Reason Code
	get_value_hex_8(section, "actionReturnReasonCode",
			&section_cper->ActionReturnReasonCode);

	// Read GUIDs from the JSON IR and populate the CPER structure
	string_to_guid(&section_cper->CpadPlatformId,
		       json_object_get_string(json_object_object_get(
			       section, "cpadPlatformID")));
	string_to_guid(&section_cper->CpadPartitionId,
		       json_object_get_string(json_object_object_get(
			       section, "cpadPartitionID")));
	string_to_guid(&section_cper->CpadCreatorId,
		       json_object_get_string(json_object_object_get(
			       section, "cpadCreatorID")));
	...
	if (json_object_object_get_ex(section, "cpadSectionIndex",
				      &cpad_section_index_obj)) {
		section_cper->CpadSectionDescriptorIndex =
			(UINT32)json_object_get_uint64(cpad_section_index_obj);
	}
	...
	if (additional_context_valid) {
		...
		if (json_object_object_get_ex(section, "additionalContext",
					      &additional_context_obj)) {
			const char *encoded_context =
				json_object_get_string(additional_context_obj);
			if (encoded_context == NULL) {
				cper_print_log(
			...
			INT32 decoded_len = 0;
			UINT8 *decoded_context = base64_decode(
				encoded_context, strlen(encoded_context),
				&decoded_len);
