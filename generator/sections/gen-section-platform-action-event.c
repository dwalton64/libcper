...
size_t
generate_section_platform_action_event(void **location,
				       GEN_VALID_BITS_TEST_TYPE validBitsType)
{
	UINT32 additional_context_size = 0;
	UINT32 total_section_size =
		0; // EFI_PLATFORM_ACTION_EVENT struct size + additional context size if valid
	...
	if (validBitsType == SOME_VALID) {
		...
		section_validation_bits = cper_rand() &
					  EFI_PLATFORM_ACTION_VALID_MASK;
	} else {
	...
	if (additional_context_size > 0) {
		memcpy((char *)section_cper + sizeof(EFI_PLATFORM_ACTION_EVENT),
		       generate_random_bytes(additional_context_size),
		       additional_context_size);
	}
