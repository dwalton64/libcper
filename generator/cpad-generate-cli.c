...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "--out") == 0 && i < argc - 1) {
		...
		} else if (strcmp(argv[i], "--action-ids") == 0 &&
			   i < argc - 1) {
			...
			if (num_action_ids > 0) {
				action_ids =
					malloc(sizeof(char *) * num_action_ids);
	...
	if (action_ids != NULL) {
		UINT16 expected_count = (single_section != NULL) ? 1 :
								   num_sections;
	...
	if (action_ids != NULL) {
		for (int k = 0; k < num_sections; k++) {
			...
			action_ids_u16[k] =
				(UINT16)strtoul(action_ids[k], NULL, 0);
		}
	} else {
	...
	if (single_section != NULL && sections == NULL) {
		generate_single_cpad_section_record(
			single_section, action_ids_u16[0], cper_file);

	} else if (sections != NULL && single_section == NULL) {
		// [MODIFIED] Pass the array of action IDs
		generate_cpad_record(sections, action_ids_u16, num_sections,
				     cper_file);
